#!/usr/bin/env python3
"""CLI entrypoint for the harness (never previously written - this
wires load_corpus() + a ModelAdapter + runner.run() together from the command line).
...
key touches this script. Two modes:
  ...
  full    - every item, R1+R2, n=3 samples.
...
MODELS = {
    ...
    "sarvam-m": dict(
        ...
        # Pinned per the "Pinned model IDs and dates" convention - confirmed this is exactly
    ...
    "param-1-2.9b": dict(
        ...
        # max_tokens (same lever used elsewhere in this file for similar non-stopping generation) should cut the dominant
